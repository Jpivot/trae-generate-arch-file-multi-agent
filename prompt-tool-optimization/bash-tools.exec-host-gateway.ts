...
import { detectDangerousCommand } from "../infra/exec-dangerous-command-detect.js";
...
export async function processGatewayAllowlist(
  ...
  const dangerousCmd = detectDangerousCommand(params.command);
  if (dangerousCmd.detected) {
    logInfo(`exec: dangerous command detected (gateway): ${dangerousCmd.reasons.join(", ")}`);
    params.warnings.push(`⚠️ Dangerous command detected: ${dangerousCmd.reasons.join("; ")}`);
  }
  ...
  if (requiresAsk) {
    ...
    void (async () => {
      ...
      if (decision === "deny") {
        ...
        } else if (dangerousCmd.detected) {
          deniedReason = "approval-timeout (dangerous-command-detected)";
